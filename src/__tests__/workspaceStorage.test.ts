...
describe('workspaceStorage', () => {
  ...
  it('createWorkspace 保留 modelConfigId', async () => {
  ...
  it('getEnabledWorkspaces 只返回 enabled 工作区', async () => {
    ...
    await createWorkspace({ name: 'Enabled', modelConfigId: 'model_a', enabled: true });
    await createWorkspace({ name: 'Disabled', modelConfigId: 'model_b', enabled: false });
  ...
  it('updateWorkspace 可更新 model 绑定', async () => {
    ...
    const workspace = await createWorkspace({ name: 'Workspace', modelConfigId: 'model_a', enabled: true });

    await updateWorkspace(workspace.id, { modelConfigId: 'model_b' });
